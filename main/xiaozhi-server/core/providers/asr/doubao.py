...
class ASRProvider(ASRProviderBase):
    def __init__(self, config: dict, delete_audio_file: bool):
        ...
        # 确保输出目录存在
        # TODO 生产环境应该将这个目录设置为外部存储，比如OSS
        os.makedirs(self.output_dir, exist_ok=True)

    def save_audio_to_file(self, opus_data: List[bytes], session_id: str) -> str:
        """
        将用户输入的Opus音频数据解码并保存为WAV文件
        开发环境保存到本地，生产环境应该保存到OSS
        """
        file_name = f"asr_{session_id}_{uuid.uuid4()}.wav"
        file_path = os.path.join(self.output_dir, file_name)

        # 用于解码Opus音频数据。16000 表示采样率为16kHz，1 表示单声道
        ...
        for opus_packet in opus_data:
            try:
                # 960 表示每个解码帧包含960个采样点，对应60ms的音频数据
    ...
    async def _send_request(self, audio_data: List[bytes], segment_size: int) -> Optional[str]:
        ...
        try:
            ...
            # async with 异步地建立一个WebSocket连接，确保在使用完WebSocket连接后会自动关闭
            async with websockets.connect(self.ws_url, additional_headers=auth_header) as websocket:
                ...
                if 'payload_msg' in result and result['payload_msg']['code'] != self.success_code:
                    ...
                    return None
                # self.slice_data 方法将音频数据分割成指定大小的块
    ...
    def read_wav_info(data: io.BytesIO = None) -> (int, int, int, int, int):
        with io.BytesIO(data) as _f:
            ...
            n_channels, samp_width, framerate, n_frames = wave_fp.getparams()[:4]
            wave_bytes = wave_fp.readframes(n_frames)
        return n_channels, samp_width, framerate, n_frames, len(wave_bytes)
