...
class WebSocketServer:
    def __init__(self, config: dict):
        ...
        # 创建vad/asr/llm/tts/memory/intent实例
    ...
    async def _handle_connection(self, websocket):
        ...
        # 创建ConnectionHandler时传入当前server实例，ws创建一个Connection处理器，存储client状态
        # TODO 入参是对象引用，多个连接会共享同一个memory对象，会状态混乱
        handler = ConnectionHandler(self.config,
                                    self._vad,
                                    self._asr,
                                    self._llm,
                                    self._tts,
