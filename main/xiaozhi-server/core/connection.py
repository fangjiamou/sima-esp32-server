...
class ConnectionHandler:
    def __init__(self, config: Dict[str, Any], _vad, _asr, _llm, _tts, _memory, _intent):
        ...
        # 客户端打断状态控制标记 TODO 这个标记设计不够优雅
        self.client_abort = False
        # 客户端监听模式，手动or自动，手动就不需要server VAD
        ...
        # 创建两个队列，分别用于存储TTS任务和音频播放任务
        self.tts_queue = queue.Queue()
        self.audio_play_queue = queue.Queue()
        # 创建一个线程池，最大工作线程数为10
        # TODO 每个连接都要创建一个线程池，会不会有性能问题
    ...
    async def handle_connection(self, ws):
        try:
            ...
            # TODO 这里应该重新生成一个memory对象
            self.memory.init_memory(device_id, self.llm)
            self.intent.set_llm(self.llm)

            # Load private configuration if device_id is provided
            #  TODO 这里要把用户自定义配置的逻辑独立出来
            b_use_private_config = self.config.get("use_private_config", False)
            self.logger.bind(tag=TAG).info(f"b_use_private_config: {b_use_private_config}, device_id: {device_id}")
            if b_use_private_config and device_id:
                try:
    ...
    def _tts_priority_thread(self):
        while not self.stop_event.is_set():
            ...
            try:
                ...
                # 如果没有中途打断就发送语音，把opus数据放入audio待播放队列
                # TODO 这里做的是客户端关键字打断
