...
def check_config_file():
    ...
    global default_config_file
    if not 'data' in user_config_file:
        return
    
    # ./data/config.yanl
    user_config = read_config(get_project_dir() + user_config_file)
    # ./config.yaml
    default_config = read_config(get_project_dir() + default_config_file)
    
    # 对比用户配置和默认配置，查出用户缺失的配置项
    missing_keys = find_missing_keys(default_config, user_config)
